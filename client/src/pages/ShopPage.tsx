    ...
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 bg-champagne">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-xl text-foreground/70 max-w-3xl mx-auto leading-relaxed" data-testid="text-page-subtitle">
          ...
          {shopProducts.filter(p => p.type === "bag").map((product, index) => (
            ...
            >
              ...
              <div className="glass-card rounded-2xl p-6 shadow-md">
                <h3 className="font-serif text-2xl font-semibold text-onyx mb-2" data-testid={`text-product-name-${product.id}`}>
                  {product.name}
                </h3>
                ...
                <p className="text-sm text-foreground/70 mb-6" data-testid={`text-product-description-${product.id}`}>
                  {product.description}
                </p>
                
                <div className="flex items-center justify-between pt-4 border-t border-white/20">
                  <div className="text-3xl font-bold text-onyx" data-testid={`text-product-price-${product.id}`}>
                    ${product.price}
                  </div>
                  <button
                    onClick={handlePurchase}
                    className="rounded-full bg-onyx text-white px-6 py-2 font-medium hover-elevate active-elevate-2 transition-all duration-200"
        ...
        <motion.div
          ...
          className="relative"
        >
          <div className="absolute -inset-1 bg-gradient-to-r from-gold/30 via-gold/20 to-gold/30 rounded-2xl blur-lg" />
          <div className="relative glass-card rounded-2xl p-8 shadow-xl border-2 border-gold/40">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
              ...
              <div>
                <div className="inline-block bg-gold text-onyx px-4 py-1 rounded-full text-sm font-semibold mb-4 shadow-md" data-testid="badge-bundle-savings">
                  Best Value - Save $98
                </div>
                
                <h2 className="font-serif text-4xl font-bold text-onyx mb-4" data-testid="text-bundle-title">
                  Trio Bundle
                </h2>
                ...
                <div className="flex items-center justify-between pt-6 border-t border-white/20">
                  <div>
                    ...
                    <div className="text-4xl font-bold text-onyx" data-testid="text-bundle-price">
                      $499
                    </div>
                  </div>
                  <button
                    onClick={handlePurchase}
                    className="rounded-full bg-gold text-onyx px-8 py-3 font-semibold text-lg hover-elevate active-elevate-2 transition-all duration-200 shadow-lg"
