...
// ===== LUNA AI ASSISTANT DRAFTS =====
// Stores all content drafts generated by Luna (Telegram bot assistant)
export const lunaDrafts = pgTable("luna_drafts", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  title: varchar("title").notNull(),
  platform: varchar("platform").notNull(), // linkedin, video_script, instagram, twitter, etc.
  content: text("content").notNull(),
  telegramChatId: varchar("telegram_chat_id"), // For tracking which chat generated it
  createdAt: timestamp("created_at").defaultNow(),
}, (table) => [
  index("idx_luna_drafts_platform").on(table.platform),
  index("idx_luna_drafts_created").on(table.createdAt),
]);

export const insertLunaDraftSchema = createInsertSchema(lunaDrafts).omit({ id: true, createdAt: true });
export type InsertLunaDraft = z.infer<typeof insertLunaDraftSchema>;
export type LunaDraftDB = typeof lunaDrafts.$inferSelect;